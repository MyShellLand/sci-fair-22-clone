...
async def main(): # GUI here
    ...
    while True:
        ...
        if userInput == "tr":
            ...
            message = USR_NODE.create_block(current_fake_work_data,current_fake_team_data,number_of_winners=1)
            print(f"message generated:{message}")

        elif userInput == "sbl":
            winners = USR_NODE.winners
            if winners.count(str(USR_NODE.pk)) > 0:
            ...
            wd: allows you to simulate work being done by nodes
            ba: find a miner's balance
            """)

        elif userInput == "ba":
            print(USR_NODE.find_balance(str(USR_NODE.pk)))
